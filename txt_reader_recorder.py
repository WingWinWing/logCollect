...
class TxtReader(object):
    ...
    def _get_value_by_patten(self, line, patten):
        # print "line = {}".format(line)
        split_comma_list = line.split(',')
        for item in split_comma_list:
            if patten in item:
                item = item.replace('.', '')
                return int(item.split(patten)[-1])
            else:
                continue
        return None
    ...
    def _get_time_offset(self, line):
        # print "line = {}".format(line)
        split_comma_list = line.split(',')
        for item in split_comma_list:
            if 'mTimeOffset =' in item:
                item = item.replace('.', '')
                return int(item.split('mTimeOffset =')[-1])
            else:
                continue
        return None
    ...
    def gen_csv(self, output_path):
        ...
        upper_limit = 20000
        pts_result = self._gen_result() # 返回 dictionary;

        items = pts_result.items()      # dictionary 转换成数组 List;
        items.sort()
